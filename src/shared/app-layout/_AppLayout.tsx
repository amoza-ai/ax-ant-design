...
import { Avatar, Badge, Button, Dropdown, Input, Layout, Space, Tooltip, Typography, theme } from 'antd'
import {
  AppstoreOutlined,
  BellOutlined,
  CheckSquareOutlined,
  ...
  return (
    <Layout className="app-layout">
      ...
      >
        <Space size="middle" align="center" className="app-layout__top-left">
          ...
          <Dropdown
            menu={{
              items: [
                { key: 'p1', label: 'Project Name' },
                { key: 'p2', label: 'Other Project' },
              ],
            }}
            trigger={['click']}
          >
      ...
      <Layout className="app-layout__middle">
        ...
        >
          <nav className="app-layout__rail-nav">{primaryNav.map((it) => renderRailItem(it, 'left'))}</nav>
        </Sider>
        ...
        >
          <nav className="app-layout__rail-nav">{tools.map((it) => renderRailItem(it, 'right'))}</nav>
        </Sider>
      </Layout>
      ...
      >
        ...
        <Space size="middle">
          <Space size={6}>
            ...
            <Text type="secondary" style={{ fontSize: token.fontSizeSM }}>
              {t('status.lastSaved', { time: '12m ago' })}
            </Text>
          </Space>
          <Text type="secondary" style={{ fontSize: token.fontSizeSM }}>
            v0.1.0
